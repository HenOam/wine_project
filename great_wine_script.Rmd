...
quality_data <- country_points %>% group_by(country) %>% slice(which.max(points)) %>% filter(points >= 90) 
...
ggplot(quality_data, aes(country, points, size = price)) + geom_point() + theme_bw() + theme(plot.title = element_text(hjust = 0.5, colour = "red", face = "bold"), axis.text.x = element_text(size = 10, colour = "black", angle = 40, vjust = 1, hjust = 1)) + labs(title = "Highest points vs Country of production", x = "Country of production", y = "Points")
```


```{r}
# data sorted in descending order to quickly see the countries with the highest points among the producers that meet the criteria
